...
export function WordResultsPanel({ cards, hasSequence, level }: WordResultsPanelProps) {
  const [page, setPage] = useState(0)
  const pageSize = level === 1 ? 8 : 16
  const totalPages = Math.max(1, Math.ceil(cards.length / pageSize))
  const currentPage = Math.min(page, totalPages - 1)
  const pageCards = cards.slice(currentPage * pageSize, currentPage * pageSize + pageSize)
  ...
  return (
    <div className="mt-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {pageCards.map((card) => (
          <WordCardView key={card.lemmaId} card={card} />
        ))}
      </div>
      {totalPages > 1 && (
